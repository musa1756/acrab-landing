...
`/buy` остаётся статическим Astro shell с React island. Сценарий: email OTP
существующего аккаунта, проверка кода, загрузка цен, выбор тарифа, создание
платежа кнопкой «Оплатить» и переход по HTTPS-ссылке, которую вернула функция
`tochka-payment`. Отдельного чекбокса оферты нет: нажатие «Оплатить» означает
